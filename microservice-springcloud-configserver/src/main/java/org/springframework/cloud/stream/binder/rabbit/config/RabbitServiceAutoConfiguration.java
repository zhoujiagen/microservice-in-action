/*
 * Copyright 2015-2017 the original author or authors. Licensed under the Apache License, Version
...
public class RabbitServiceAutoConfiguration {
  ...
  // */
  // @Configuration
  // @ConditionalOnProperty(value = "spring.cloud.stream.overrideCloudConnectors", havingValue =
  ...
  // cloud.getSingletonServiceConnector(ConnectionFactory.class,
  ...
  static void configureCachingConnectionFactory(CachingConnectionFactory connectionFactory,
      ConfigurableApplicationContext applicationContext, RabbitProperties rabbitProperties)
      throws Exception {
    ...
    if (rabbitProperties.getCache().getConnection().getSize() != null) {
      connectionFactory.setConnectionCacheSize(rabbitProperties.getCache().getConnection()
          .getSize());
    }
    if (rabbitProperties.getCache().getChannel().getCheckoutTimeout() != null) {
      connectionFactory.setChannelCheckoutTimeout(rabbitProperties.getCache().getChannel()
          .getCheckoutTimeout().getSeconds() * 1000l); // MARK(zhoujiagen) FIX BUG
    }
